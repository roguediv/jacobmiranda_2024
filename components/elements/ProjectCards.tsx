...
const ProjectCards: React.FC<props> = ({ className = "", projects = [] }) => {
  useEffect(() => {
    ...
    const observer4 = ObserveElement(
      ".ProjectCards .trigger4",
      (e) => {
        if (sctElement) {
    ...
    return (
      <div className="ProjectCards projects">
        {projects.map((item, i) => {
          return (
            <Link
              href={`/projects/${item.body.title}`}
              className={`trigger${i + 1}`}
              key={item.body.title}
            >
              <div className={`background`}>
                <Image
                  className="imgBackground"
                  src={`/images/projects/${item.body.title}/preview.png`}
                  width={1000}
                  height={1000}
            ...
            if (i == 0) {
              return (
                <CardProject
                  ...
                  displayTitle={project?.body.displayTitle}
                  desc={project?.body.desc}
                  src1={`/images/projects/${project.body.title}/background.png`}
                  src2={`/images/projects/${project.body.title}/bodyLogo.png`}
                />
              );
            }
            return (
              <CardProject
                className={`trigger${i * 2}`}
                key={project.body.title}
                title={project?.body.title}
                displayTitle={project?.body.displayTitle}
                desc={project?.body.desc}
      ...
      <Link href={"/projects"}>
        <button className="more">
          <div className="svg-holder">
            ...
            >
              <path
                d="M16.5 1H2.5L24.5 23L2.5 45.5H16.5L38 23L16.5 1Z"
                fill="#CF12BC"
                stroke="#CF12BC"
                strokeWidth="2"
              />
